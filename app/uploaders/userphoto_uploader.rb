USERPHOTO_UPLOADER_ALLOW_USERPHOTO_VERSION_NAMES = ["720x240"]
class UserphotoUploader < CarrierWave::Uploader::Base
  ...
  def url(version_name = "")
    ...
    if not version_name.in?(USERPHOTO_UPLOADER_ALLOW_USERPHOTO_VERSION_NAMES)
      # To protected version name using, when it not defined, this will be give an error message in development environment
      raise "UserphotoUploader version_name:#{version_name} not allow."
    end
